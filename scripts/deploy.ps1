...
try {
    ...
    if (-not (Get-GameConfig -GameId 'arx-fatalis')) {
        throw "cameraunlock-core/data/games.json has no 'arx-fatalis' entry. Add it in cameraunlock-core and update the submodule here - passing a game directory does not work around it, because the running-game check and the exe-directory lookup read the entry too."
    }

    $result = Invoke-DevDeployASILoader `
        -GameId 'arx-fatalis' `
        -GameDisplayName 'Arx Fatalis' `
        -BuildOutputPath (Join-Path $projectRoot "bin\$Configuration") `
        -ModDllName 'ArxFatalisHeadTracking.asi' `
        -VendorLoaderDll (Join-Path $projectRoot 'vendor\ultimate-asi-loader\dinput8.dll') `
        -AsiLoaderName 'dinput.dll' `
        -GivenPath $GamePath

    Write-Host ''
    Write-Host "Deployed to $($result.ExeDir). Launch Arx Fatalis to use head tracking." -ForegroundColor Green
} catch {
